...
<View style={[styles.buttonContainer, { height: "20%", justifyContent: "space-evenly" }]}>
  <Button
    mode="contained"
    onPress={async () => {
      try {
        navigation.navigate("TermsScreen"); // Redireciona para a tela de Termos
      } catch (error) {
        console.error("Erro ao ir aos termos:", error);
      }
    }}
  ...
  <Button
    ...
    onPress={async () => {
      try {
        ...
        navigation.navigate("WelcomeScreen"); // Redireciona para a tela Welcome
      } catch (error) {
